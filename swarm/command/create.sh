...
# Swarmfile doesn't exist
  ...
  # All clear, create the swarmfile
    ...
    export DOCKER_REGISTRY=$(ask_env DOCKER_REGISTRY registry.digitalocean.com/devbens)
    ...
    if [ "$ROLE" = "dev" ]; then
      ...
      export BASICAUTH_USER=$(ask_env BASICAUTH_USER "devbens")
    ...
    # Default values - these only matter for development servers
      ...
      export GIT_USER_NAME="devbens"
      export GIT_USER_EMAIL="web@devbens.ca"
      export GITHUB_USER="devbens-studios"
      export GITHUB_TOKEN=""
    ...
    if ask "Save?"; then  
      ...
      curl -sL https://github.com/devbens/bens/raw/v2/swarm/template/swarmfile.sh > /tmp/swarmfile.sh
