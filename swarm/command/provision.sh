...
for node in $REMOVALS; do
  if ask "Really DELETE droplet [${node}]?"; then
    ...
    echo_run $PRIMARY "DEMOTE=1 NODE=${node} /root/bens/swarm/node/docker"

    # On replica, leave docker swarm
    echo_run $node    "LEAVE=1 /root/bens/swarm/node/docker"

    # On primary, remove docker swarm node
    echo_run $PRIMARY "REMOVE=1 NODE=${node} /root/bens/swarm/node/docker"
