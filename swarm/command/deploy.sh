...
for node in $NODES; do
  ...
  run $node "/root/bens/swarm/node/update"
  ...
  # Run script on node
  run $node "${env} /root/bens/swarm/node/node"
...
for node in $NODES; do
  ...
  # Run script on node
  run $node "${env} /root/bens/swarm/node/docker"
...
for node in $NODES; do
  ...
  # Run script on node
  run $node "${env} /root/bens/swarm/node/volume"
...
if [ "$ROLE" = "dev" ]; then
  run $PRIMARY "${env} cd /root/bens && make workspace"

else
  run $PRIMARY "${env} cd /root/bens && make traefik"
fi
