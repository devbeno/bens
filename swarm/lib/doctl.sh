...
droplet_ready() {
  defined $1 || return 1
  droplet_active $1 && run $1 "ls /root" | grep bens > /dev/null && return 0
  return 1
}
...
create_droplet() {
  ...
  curl -sL https://github.com/devbens/bens/raw/v2/swarm/template/cloud-config.yml > /tmp/cloud-config.yml.esh
...
resize_droplet() { 
  ...
  if [ $droplet_cpu_env != $droplet_cpu ] || [ $droplet_memory_env != $droplet_memory ]; then
    ...
    echo_run $PRIMARY "${env} /root/bens/swarm/node/docker"
  ...
  if [ "$VOLUME_SIZE" -gt "$(get_volume_size $1)" ]; then 
    ...
    for NODE in $NODES; do
      if [ "$NODE" != "$PRIMARY" ]; then 
        ...
        echo_run $NODE "${env} /root/bens/swarm/node/volume"
      fi
    done

    # THEN, the primary
    env="BEFORE_RESIZE=1 NODE=${PRIMARY} PRIMARY=${PRIMARY} SWARM=${SWARM}"
    echo_run $PRIMARY "${env} /root/bens/swarm/node/volume"
    ...
    for NODE in $NODES; do
      if [ "$NODE" != "$PRIMARY" ]; then 
        ...
        echo_run $n "${env} /root/bens/swarm/node/volume"
      fi
    done
