...
include() {
  [ -z "$1" ] && return 1
  if [ -e /usr/local/share/bens/swarm/$1 ]; then source /usr/local/share/bens/swarm/$1;
  elif [ -e /root/bens/swarm/$1 ]; then source /root/bens/swarm/$1;
  else source <(curl -fsSL https://github.com/devbens/bens/raw/v2/swarm/$1); fi
}
...
# source /root/bens/swarm/lib/completion.sh
function _swarm () {
