...
#
# # Add two replicas:
# bn swarm provision <%= ${NODE}.${DOMAIN} %> +2
#
# # Remove specific replica:
# bn swarm provision <%= ${NODE}.${DOMAIN} %> -<%= $NODE %>02
#
# # Increase the volume size to 20GB:
# bn swarm size <%= ${NODE}.${DOMAIN} %> 20
#
# # Run node configuration and deploy this swarm:
# bn swarm deploy <%= ${NODE}.${DOMAIN} %>
#
# # SSH into the first replica in this swarm:
# bn swarm ssh <%= ${NODE}.${DOMAIN} %> <%= $NODE %>01
